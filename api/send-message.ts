...
// Map frontend message format to Gemini's format
const buildGeminiHistory = (messages: Message[]) => {
  // Filter out non-text messages and forms, as the AI should only see the text conversation flow.
  return messages
...
export default async function handler(
  ...
  try {
    const { inputText, history } = request.body;
    ...
    const sources = []; // Search is disabled, so sources are always empty.

    return response.status(200).json({ text: responseText, requestType, sources });
    
  } catch (error) {
    console.error('Error in send-message function:', error);
    const message = error instanceof Error ? error.message : 'An unknown internal error occurred.';
    return response.status(500).json({ message: `AI service error: ${message}` });
  }
}
